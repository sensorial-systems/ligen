use crate::prelude::*;
use rustpython_parser::ast::{StmtClassDef, Arguments, Expr, Stmt};
...
impl<T> Parser<&StmtClassDef<T>> for InterfaceParser {
    ...
    fn parse(&self, input: &StmtClassDef<T>) -> Result<Self::Output> {
        ...
        let constants = self.parse_constants(&input.body)?;
        let functions = self.parse_functions(&input.body)?;
        let methods = self.parse_methods(&input.body)?;
...
impl InterfaceParser {
    ...
    fn parse_methods<T>(&self, statements: &[Stmt<T>]) -> Result<Vec<Identifier>> {
