...
impl Parser<Attributes> for RustAttributesParser {
    fn parse(&self, input: impl AsRef<str>, config: &Config) -> Result<Attributes> {
        ...
        let attributes = syn::parse_str::<
            syn2::punctuated::Punctuated<IntermediaryAttribute, syn::token::Comma>,
        >(input)
...
impl Transformer<syn::punctuated::Punctuated<IntermediaryAttribute, syn::token::Comma>, Attributes>
    for RustAttributesParser
{
    fn transform(
        &self,
        input: syn::punctuated::Punctuated<IntermediaryAttribute, syn::token::Comma>,
        config: &Config,
    ) -> Result<Attributes> {
        let mut attributes = Vec::new();
...
impl Transformer<syn::punctuated::Punctuated<syn::Expr, syn::token::Comma>, Attributes>
    for RustAttributesParser
{
    fn transform(
        &self,
        ...
        let attributes = input
            .into_iter()
            .map(|input| self.attribute_parser.transform(input, config))
            .collect::<Result<Vec<_>>>()?;
        Ok(Attributes { attributes })
    }
}

impl Transformer<syn::punctuated::Punctuated<syn::Meta, syn::token::Comma>, Attributes>
    for RustAttributesParser
{
    fn transform(
        &self,
        input: syn::punctuated::Punctuated<syn::Meta, syn::token::Comma>,
        config: &Config,
    ) -> Result<Attributes> {
        let attributes = input
            .into_iter()
            .map(|nested_meta| self.attribute_parser.transform(nested_meta, config))
            .collect::<Result<Vec<_>>>()?;
        Ok(Attributes { attributes })
    }
}
...
mod test {
    ...
    fn parse_expressions() -> Result<()> {
        assert_eq(
            RustAttributesParser::default(),
            mock::parse_expressions(),
            r#"error("the {} field name: '{}' is invalid, path: {:?}", self.0.field_type, self.0.field_name, self.0.path)"#,
        ) // we need to make expressions valid.
    }
}
