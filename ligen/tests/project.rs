...
fn project(path: PathBuf) {
    ...
    let mut project = Project::try_from(project).unwrap();
    ...
    assert_eq!(project.root_module.name, "example".into());
    // println!("{:#?}", project);
...
fn find_absolute_path(project: &Project) -> Path {
    ...
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"root::time::instant::Instant".into()), "Failed in absolute path case.");
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"self::time::instant::Instant".into()), "Failed in self path case.");
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"time::instant::Instant".into()), "Failed in sub-module case.");
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"Instant".into()), "Failed in import case.");
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"RenamedInstant".into()), "Failed in renamed import case.");
    assert_eq!(Some("example::time::instant::Instant".into()), module_visitor.find_absolute_path(&"time::Instant".into()), "Failed in re-exported case.");
